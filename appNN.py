...
def pre_ann(data):
    loaded_model = tf.keras.models.load_model("models/best_neural_network_model3.h5")

    age = datetime.datetime.now().year - int(data['year'])
    input_data = np.array([[int(data['km-driven']), float(data['average']), int(data['engine-cc']), float(data['new-price']), age]])

    scaler = StandardScaler()
    input_data_normalized = scaler.fit_transform(input_data)

    predictions = loaded_model.predict(input_data_normalized)
...
def check_price():
    ...
    print(nn, "Lakhsssssssssssssssssssss")

    if data["model-name"] == "1":
        return jsonify({'result':linear, 'arr': {'Random Forest': rf, 'Decision Tree': dT, 'Linear': linear}})
    elif data["model-name"] == "2":
        return jsonify({'result':rf, 'arr': {'Random Forest': rf, 'Decision Tree': dT, 'Linear': linear}})
    elif data["model-name"] == "3":
        return jsonify({'result':dT, 'arr': {'Random Forest': rf, 'Decision Tree': dT, 'Linear': linear}})
    else:
        return jsonify({'result':linear, 'arr': {'Random Forest': rf, 'Decision Tree': dT, 'Linear': linear}})
